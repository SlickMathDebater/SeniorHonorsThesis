...
\colorcomment{RoyalBlue}{In The Analytical Theory of Heat \colorcomment{red}{citationstuff} Fourier said \colorcomment{red}{quote} 
Now, observe Dini's Test;}

\begin{thm}
\colorcomment{red}{If $f \in L^1[0, 2\pi]$, $x_0 \in f^{-1}(\R)$, and $F(x) = \frac{f(x+x_0) - f(x_0)}{x}$ is Riemann-integrable over some open neighborhood of $x_0$, then the Fourier series of $f$ converges at $x_0$.}
\end{thm}
...
So, the condition for applying Dini's test is that $f(x+x_0) - f(x_0)$ approaches $0$ at least as fast as a linear function does.
...
\begin{proof}
We need to show that given a nonempty open set $U$, that $\bigcap E_n \cap U$ is nonempty.
...
\colorcomment{Thistle}{I'm not 100\% sure about the above line. I think I hit everything, but I could easily have missed something.}
...
The above set is the set of functions, $f \in C(T)$, such that $Mf >n$ for all $n \in \N$.
